...
import { Sparkles, X, Send, RotateCcw, Copy, Check, Brain } from "lucide-react";
...
export function FloatingCopilot() {
  ...
  const systemPrompt = React.useMemo(() => {
    ...
    return [
      ...
      `Ты умеешь отмечать посещаемость учеников (mark_attendance: present/пришел, absent/не пришел, late/опоздал) и просматривать журнал (list_lessons). Например, когда пользователь просит «Отметь Алишера как пришедшего, а Мадину как опоздавшую в группе English A1», вызови mark_attendance.`,
...
const MINI_ICONS: Record<string, React.ReactNode> = {
  "openrouter-free": <Brain className="h-3.5 w-3.5" />,
  "gemma-4-26b-free": <Sparkles className="h-3.5 w-3.5" />,
};
